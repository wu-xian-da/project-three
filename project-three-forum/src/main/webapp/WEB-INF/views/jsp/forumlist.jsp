...
<body>
	...
	<table width="100%;" style="font-size: 8px;" class="td">
		...
		<!-- 首页 -->
		<td width="35px;"><a href="${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?pn=0&ps=${page.pageSize}">首页</a></td>
		<!-- 上一页 -->
		<td width="50px;">
		<a id="shangyiye" href="${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?pn=${bianPageShang}${jspurl}&ps=${page.pageSize}">上一页
		...
		<td align="center">
			...
			<c:forEach begin="0" end="${pageNo-1}" step="1" var="pageIndex">
				...
				href="${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?pn=${pageIndex}&ps=${page.pageSize}">${pageIndex+1}</a>
			</c:forEach>	
			</c:if>
		</td>
		<!-- 下一页 -->
		<td width="50px;">
		<a id="xiayiye" href="${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?pn=${bianPageXia}${jspurl}&ps=${page.pageSize}">
		<input id="pagePnXia" type="hidden" name="pn" value="${bianPageXia}"/>
		<input id="totalPage" type="hidden" name="pn" value="${pageNo}"/>下一页</a>
		</td>
		<!-- 末页 -->
		<td width="35px;"><a href="${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?pn=${pageNo-1}&ps=${page.pageSize}">末页</a></td>
		</tr>
	</table>
</body>
...
<script type="text/javascript">
	...
	$(function(){
		$("#page_ps_select").change(function(){
			window.location.href = '${pageContext.request.contextPath}/forum/notes/forumlistByparentmodules/${parentmodules}?ps='+$(this).val();
		});
	});
</script>
